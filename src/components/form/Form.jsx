...
const Form = () => {
  ...
  return (
    <section id='contact' className='w-full h-screen flex flex-col pt-20 items-center bg-green-500'>
        ...
        <form id='form' ref={form} onSubmit={handleSubmit} method="POST" className='w-3/5 h-4/5'>
          <fieldset className='border-2 border-black flex flex-col h-full items-center justify-around shadow-2xl'>
            ...
            <textarea className='w-3/4 rounded-sm border-2 border-black p-2' name="message" cols="30" rows="10" placeholder='Escribe tu mensaje acá' required></textarea>
            <button type='submit' className='bg-blue-400 w-1/4 h-10 rounded-md border-2 border-black hover:bg-slate-700 hover:text-blue-200 hover:font-bold'>Enviar</button>
          </fieldset>
        </form>
    </section>
  );
